...
from polls.analysis.ml_rent_predict import train_rent_model, calculate_predicted_rents_for_property
...
def generate_ml_visualizations(model, X_train, y_train, prefix="ml"):
    output_dir = os.path.join(settings.BASE_DIR, 'polls', 'static', 'dashboard_charts')
    os.makedirs(output_dir, exist_ok=True)
    print(f"[INFO] Saving ML charts to: {output_dir}")
    ...
    r2 = r2_score(y_train_orig, y_pred_orig)
    mae = mean_absolute_error(y_train_orig, y_pred_orig)
    rmse = root_mean_squared_error(y_train_orig, y_pred_orig)

    # Save model evaluation metrics as text image
    fig, ax = plt.subplots(figsize=(4, 3)) 
    ax.axis('off')
    ax.text(0.5, 0.75, f"R²: {r2:.4f}", fontsize=18, ha='center', va='center')
    ax.text(0.5, 0.50, f"MAE: €{mae:,.0f}", fontsize=18, ha='center', va='center')
    ax.text(0.5, 0.25, f"RMSE: €{rmse:,.0f}", fontsize=18, ha='center', va='center')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'chart_{prefix}_metrics.png'), bbox_inches='tight')
    plt.close()
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        plt.title(f"{prefix.upper()} Feature Importance")
        plt.tight_layout()
        file_path = os.path.join(output_dir, f'chart_{prefix}_feature_importance.png')
        plt.savefig(file_path, bbox_inches='tight')
        plt.close()
        print(f"[SAVE] Feature Importance chart saved to: {file_path}")
    else:
        print("[WARN] Model has no feature_importances_ attribute.")

    # Actual vs Predicted scatter plot
    plt.figure(figsize=(6, 6))
    plt.scatter(y_train, y_pred, alpha=0.7)
    plt.plot([y_train.min(), y_train.max()], [y_train.min(), y_train.max()], '--r')  
    plt.title(f"{prefix.upper()} Actual vs Predicted")
    plt.xlabel("Actual Values")
    plt.ylabel("Predicted Values")
    plt.tight_layout()
    file_path = os.path.join(output_dir, f'chart_{prefix}_pca.png')  
    plt.savefig(file_path, bbox_inches='tight')
    plt.close()
    print(f"[SAVE] Actual vs Predicted scatter saved to: {file_path}")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Initialize rent prediction model
        self.initialize_rent_model()
    ...
    def train_model(self):
        ...
        if 'price' in df.columns:
            ...
            self.stdout.write(f"Filtered prices outside 1%-99% quantiles: kept {len(df)} rows.")
        ...
        generate_ml_visualizations(self.model, X_train_scaled, y_train, prefix="sale")
    ...
    def initialize_rent_model(self):
        """Initialize rent prediction model"""
        self.stdout.write("Initializing rent prediction model...")
        
        # Get ALL rent data for training
        rent_data = pd.DataFrame(list(Propertyrent.objects.all().values()))       

        for col in rent_data.columns:
            if len(rent_data[col].dropna()) > 0:
                sample_val = rent_data[col].dropna().iloc[0]
                if isinstance(sample_val, Decimal):
                    rent_data[col] = rent_data[col].astype(float)
        
        if 'publish_date' in rent_data.columns:
            rent_data['publish_date'] = pd.to_datetime(rent_data['publish_date'])
            if rent_data['publish_date'].dt.tz is not None:
                rent_data['publish_date'] = rent_data['publish_date'].dt.tz_localize(None)
        
        # Train the rent model
        self.rent_model, self.rent_scaler, self.trend_model, self.rent_min_date, \
        self.rent_split_idx, self.rent_encoders, self.X_rent_train, self.y_rent_train = train_rent_model(rent_data)
        generate_ml_visualizations(self.rent_model, self.X_rent_train, self.y_rent_train, prefix="rent")

            
        self.stdout.write(self.style.SUCCESS("Rent prediction model initialized!"))
    
    def predict_future_rents(self, property_features, years=5):
        """Predict future rents using ML model"""
        pred_features = {}
        required_features = ['bedrooms', 'bathrooms', 'Property_Type', 'county', 'ecode']
        for feature in required_features:
            if feature in property_features:
                pred_features[feature] = property_features[feature]
        
        for col, encoder in self.rent_encoders.items():
            if col in pred_features:
                val = str(pred_features[col])
                if val in encoder.classes_:
                    pred_features[col] = encoder.transform([val])[0]
                else:
                    pred_features[col] = 0
        
        # Get predicted rent
        annual_rents = calculate_predicted_rents_for_property(
            pred_features,
            self.rent_model,
            self.rent_scaler,
            self.trend_model,
        ...
        return annual_rents[:years]

    def calculate_all_risk_yield_plus(self, initial_investment, monthly_rent, future_price, property_features):
        """Calculate all_risk_yield_plus using ML predicted rents"""
        ...
        # Predicting rent prices using ML
        annual_rents = self.predict_future_rents(property_features, holding_period)
        
        # If ML prediction fails, use the original calculation method
        if not annual_rents or len(annual_rents) < holding_period:
            ...
            # Calculate cash flow using ML predicted rent
            for year_rent in annual_rents:
                maintenance_cost = float(year_rent) * maintenance_percentage
                net_cash_flow = float(year_rent) - maintenance_cost
                cash_flows.append(net_cash_flow)
    ...
    def update_yields_plus(self, rent_map):
        ...
        for prop in sales:
            try:
                ...
                future_price = self.predict_future_price(property_features, years=5)
                
                # Try to use ML calculations, and if that fails fall back to the original method
                try:
                    ...
                    # If the ML method fails, fall back to the original method
                    all_risk_yield_plus_val = self.original_calculate_all_risk_yield_plus(
                ...
                self.stdout.write(self.style.WARNING(f"Error processing property {prop.id}: {ex}"))
                
                # Try using the original method
                try:
                    if prop.price is not None and (prop.county, prop.bedrooms) in rent_map:
                        price_float = float(prop.price)
                        rent_float = rent_map[(prop.county, prop.bedrooms)]
                        
                        if price_float > 0 and rent_float > 0:
                            ...
                            # Save the results
                            if all_risk_yield_plus_val <= 9999.99 and all_risk_yield_plus_val >= -9999.99:
                                prop.all_risk_yield_plus = Decimal(f"{all_risk_yield_plus_val:.2f}")
                                prop.save(update_fields=['all_risk_yield_plus'])
                                updated_count += 1
                except Exception:
    ...
    def original_calculate_all_risk_yield_plus(self, initial_investment, monthly_rent, future_price):
        """Original calculation method as a fallback"""
        maintenance_percentage = 0.34
        holding_period = 5
        income_growth_rate = 0.02
        rental_review_period = 1
